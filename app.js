...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function salvaRicettaSupabaseBase({
    ...
    const payload = {
      ...
      tipo_ricetta: tipoRicetta || "piatto",
      attivo: true,
    };
  ...
  async function caricaRicettaInForm(ricettaId) {
    ...
    if (ricettaTipoSelect)
      ricettaTipoSelect.value = ricetta.tipo_ricetta || "piatto";
  ...
  async function handleSalvaRicetta() {
    ...
    const tipoRicetta = ricettaTipoSelect?.value || "piatto";
  ...
  // creo il datalist per l'autocompletamento del ricettario (non obbligatorio ma lo teniamo)
  ...
  // carica ricette per il ricettario
  ...
  function renderPaginazioneRicette(listaTotale) {
    const container = document.getElementById("ricette-lista-viewer");
    const total = listaTotale.length;
    ...
    prevBtn.textContent = "◀";
    prevBtn.className = "app-button tiny";
    prevBtn.disabled = ricettarioPagina === 1;

    const nextBtn = document.createElement("button");
    nextBtn.textContent = "▶";
    nextBtn.className = "app-button tiny";
    nextBtn.disabled = ricettarioPagina === totalPages;

    const label = document.createElement("span");
    label.style = "font-size:13px;";
    label.textContent = `Pagina ${ricettarioPagina} / ${totalPages}`;

    prevBtn.addEventListener("click", () => {
      if (ricettarioPagina > 1) {
        ricettarioPagina--;
        renderRicetteViewer(ricetteCache);
      }
    });
    nextBtn.addEventListener("click", () => {
      if (ricettarioPagina < totalPages) {
        ricettarioPagina++;
        renderRicetteViewer(ricetteCache);
      }
    });
  ...
  // funzione di render lista ricette
  function renderRicetteViewer(listaOriginaria) {
    const container = document.getElementById("ricette-lista-viewer");
    if (!container) return;
    ...
    // filtro per tipo
    let lista = [...listaOriginaria];
    if (ricettarioFiltro !== "tutte") {
      lista = lista.filter((r) => (r.tipo_ricetta || "piatto") === ricettarioFiltro);
    }
    ...
    const paginata = lista.slice(start, start + RICETTARIO_PER_PAGINA);
    ...
    paginata.forEach((r) => {
      ...
      const badge =
        (r.tipo_ricetta || "piatto") === "base"
          ? `<span style="background:#2563eb;color:#fff;padding:2px 6px;border-radius:6px;font-size:11px;margin-left:6px;">Base</span>`
          : `<span style="background:#16a34a;color:#fff;padding:2px 6px;border-radius:6px;font-size:11px;margin-left:6px;">Ricetta</span>`;
      ...
      card.addEventListener("click", () => {
        mostraRicettaDettaglioViewer(r.id);
      });

      container.appendChild(card);
    });

    renderPaginazioneRicette(lista);
  }

  // dettaglio ricetta nel ricettario (solo lettura)
  async function mostraRicettaDettaglioViewer(ricettaId) {
    if (!supabase) return;
    const container = document.getElementById("ricette-lista-viewer");
    if (!container) return;
    ...
    if (errRic) {
      console.error("Errore lettura ricetta viewer:", errRic);
      alert("Errore nel caricare la ricetta");
      return;
    }
    ...
    if (errIng) {
      console.error("Errore lettura ingredienti viewer:", errIng);
    }

    container.innerHTML = "";

    const btnBack = document.createElement("button");
    btnBack.className = "app-button tiny gray";
    ...
    const badge =
      (ricetta.tipo_ricetta || "piatto") === "base"
        ? `<span style="background:#2563eb;color:#fff;padding:4px 8px;border-radius:6px;font-size:12px;">Ricetta base</span>`
        : `<span style="background:#16a34a;color:#fff;padding:4px 8px;border-radius:6px;font-size:12px;">Ricetta</span>`;

    let fotoHTML = "";
    if (ricetta.foto_url) {
      fotoHTML = `
        <img src="${ricetta.foto_url}" 
    ...
    wrapper.innerHTML = `
      ${fotoHTML}
      <h2 style="margin-bottom:4px;">${ricetta.nome}</h2>
      ${badge}
      ...
      <ul id="viewer-ingredienti" style="margin-top:6px;"></ul>
    ...
    // solo manager/admin può modificare
    if (currentUser && isManagerRole(currentUser.ruolo)) {
      const btnContainer = document.getElementById("viewer-buttons");
      const btnMod = document.createElement("button");
      btnMod.className = "app-button small blue";
      btnMod.textContent = "Modifica ricetta";

      btnMod.addEventListener("click", () => {
        ricettaDaAprireId = ricettaId;
        window.location.hash = "ricette";
      });

      btnContainer.appendChild(btnMod);
    }
  }

  // Ricerca ricette: filtro live nella lista (NON apre editor)
  if (ricetteSearchInput) {
  ...
  // Filtri tipo ricetta (se presenti nell'HTML)
  const filtriContainer = document.getElementById("ricette-filtri");
  if (filtriContainer) {
    filtriContainer.addEventListener("click", (e) => {
      const btn = e.target.closest("[data-filter]");
      if (!btn) return;
      ricettarioFiltro = btn.getAttribute("data-filter") || "tutte";
      ricettarioPagina = 1;
      renderRicetteViewer(ricetteCache);
    });
  }
