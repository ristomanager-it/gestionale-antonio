...
export async function render(container) {
  ...
  if (!user || !azienda) {
    container.innerHTML = createPageLayout({
      ...
      content: createCard({ body: "<p>Utente o azienda non validi.</p>" })
    });
    return;
  }
  ...
  if (error || !dipendente) {
    ...
    container.innerHTML = createPageLayout({
      title: "Errore",
      content: createCard({ body: "<p>Dipendente non trovato.</p>" })
    });
    return;
  }
  ...
  const fotoUrl = dipendente.foto_url || dipendente.avatar_url || "";
    ...
    <div style="max-width:900px;margin:auto;width:100%;">
      <form id="profilo-form">
        <div class="crea-grid">
          ...
          <div class="card">
            ...
            <div class="form-group">
              <label>Nome</label>
              <input id="nome" class="input" value="${escapeHtmlAttr(dipendente.nome || user.user_metadata?.nome || "")}">
            </div>

            <div class="form-group">
              <label>Cognome</label>
              <input id="cognome" class="input" value="${escapeHtmlAttr(dipendente.cognome || user.user_metadata?.cognome || "")}">
            </div>

            <div class="form-group">
              <label>Email</label>
              <input id="email" type="email" class="input" value="${escapeHtmlAttr(user.email || dipendente.email || "")}">
            </div>

            <div class="form-group">
              <label>Nuova password</label>
              <input id="password" type="password" class="input" placeholder="Lascia vuoto per non modificare">
            </div>
            ...
            <div class="form-group">
              ...
              <input id="pin" class="input" inputmode="numeric" maxlength="6" value="${escapeHtmlAttr(pinValue)}" placeholder="Inserisci un PIN da 4 a 6 cifre">
              <div style="font-size:12px;color:#64748b;margin-top:4px;">Il PIN servirà per il futuro accesso dipendente.</div>
            </div>
          ...
          <div class="card">
            ...
            <div class="form-group">
              ...
              <div style="font-size:12px;color:#64748b;margin-top:4px;">Puoi caricare una foto dalla galleria o scattarla da smartphone.</div>
            </div>
          </div>
          ...
          <div class="card">
            ...
            <div class="form-group">
              ...
              <textarea id="debolezze" class="input">${escapeHtml(profiloAI.debolezze || profiloAI.obiettivi_personali || "")}</textarea>
            </div>
            <div class="form-group">
        ...
        <div style="margin-top:20px;">
          <button class="app-button primary" id="btn-save" style="width:100%;" type="submit">
    ...
    fotoInput.addEventListener("change", () => {
      ...
      reader.onload = () => {
        fotoPreview.innerHTML = `
          <img src="${escapeHtmlAttr(String(reader.result || ""))}" alt="Foto profilo" style="width:110px;height:110px;border-radius:18px;object-fit:cover;border:1px solid #e5e7eb;background:#f9fafb;">
        `;
      };
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (file) {
      const uploaded = await uploadFotoProfilo(file, azienda.id, user.id);
      if (!uploaded.ok) {
        msg.innerHTML = "<span style='color:red;'>Errore caricamento foto profilo.</span>";
        btn.disabled = false;
    ...
    if (emailValue && emailValue !== user.email) {
      authUpdate.email = emailValue;
    }

    if (passwordValue) {
      authUpdate.password = passwordValue;
    }

    const { error: authError } = await supabase.auth.updateUser(authUpdate);

    if (authError) {
      console.error("Errore update auth:", authError);
      msg.innerHTML = `<span style='color:red;'>${escapeHtml(authError.message || "Errore aggiornamento auth")}</span>`;
      btn.disabled = false;
      btn.innerText = dipendente.profilo_completato ? "Salva modifiche" : "Completa profilo";
      return;
    }
    ...
    if (updateError) {
      console.error("Errore salvataggio profilo:", updateError);
      msg.innerHTML = "<span style='color:red;'>Errore salvataggio profilo</span>";
      btn.disabled = false;
      btn.innerText = dipendente.profilo_completato ? "Salva modifiche" : "Completa profilo";
      return;
    }
...
async function uploadFotoProfilo(file, aziendaId, userId) {
  const ext = String(file.name || "").split(".").pop() || "jpg";
  const safeExt = ext.toLowerCase().replace(/[^a-z0-9]/g, "") || "jpg";
  const path = `${aziendaId}/${userId}/profilo-${Date.now()}.${safeExt}`;

  const buckets = ["foto-profili", "avatar", "loghi-aziende"];
  let lastError = null;
  ...
  console.error("Upload foto profilo fallito:", lastError);
