export async function renderMapping(container, azienda) {
  ...
  input.addEventListener("input", async () => {
        ...
        ${data.map(p => `
          ...
          <div class="rf-search-item">
            ...
            <div class="rf-search-row">
              ...
              <button
                type="button"
                class="rf-search-action open-mapping"
                data-id="${p.id}"
              >🔍</button>
    ...
    risultati.querySelectorAll(".open-mapping").forEach(btn => {
...
async function apriMapping(box, azienda, prodottoId) {
  ...
  const giacenza = (movimenti || []).reduce((tot, mov) => {
    ...
    if (mov.tipo_movimento === "SCARICO") return tot - q;
  ...
  box.innerHTML = `
    ...
    <div class="rf-product-card">
      ...
      <div class="rf-product-section-title">Fornitori collegati</div>
      ...
      <div class="rf-mov-list">
        ...
        ${(mapping || []).length
          ...
          `).join("")
          : `<div class="rf-empty-state">Nessun fornitore collegato</div>`
        }
