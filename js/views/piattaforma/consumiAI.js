...
function usd(n) { return Number(n || 0).toFixed(3) + ' USD'; }
function eur(n) { return Number(n || 0).toFixed(2).replace('.', ',') + ' EUR'; }
...
function layout() {
  ...
  <div class="view pf-consumi">
    ...
    <style>
      ...
      .pf-box .v{font-size:20px;font-weight:700;margin-top:3px}
      .pf-box.ok .v{color:#2F7D32}
      .pf-riga{display:grid;grid-template-columns:1.6fr .7fr .7fr .9fr .9fr;gap:8px;align-items:center;
...
async function carica() {
  ...
  let costoTot = 0, saldoTot = 0, azioniMese = 0, incassoTot = 0, costoVita = 0;
  righe.forEach(r => {
    costoTot += Number(r.costo_usd_mese || 0);
    saldoTot += Number(r.saldo || 0);
    azioniMese += Number(r.azioni_mese || 0);
    incassoTot += Number(r.incassato_eur || 0);
    costoVita += Number(r.costo_usd_totale || 0);
  });
  ...
  document.getElementById('pf-totali').innerHTML =
    ...
    box('Incassato', eur(incassoTot), 'ok') +
    box('Margine', eur(incassoTot - costoVita * USD_EUR), 'ok') +
  ...
  document.getElementById('pf-aziende').innerHTML =
    '<div class="pf-riga testa">' +
      ...
      '<span class="pf-num solo-largo">Incassato</span>' +
    '</div>' +
    (righe.length ? righe.map(r =>
      '<div class="pf-riga">' +
        ...
        '<span class="pf-num solo-largo">' + eur(r.incassato_eur) + '</span>' +
      '</div>'
