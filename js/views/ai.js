...
<head>
  ...
  <meta chimport { supabase } from "../supabaseClient.js";
...
function initChat(ruolo) {
  ...
  <title>Ristoflow - la ristorazione organizzata</title>

  <link rel="icon" href="assets/favicon-192.png?v=9" />
  <link rel="apple-touch-icon" href="assets/favicon-192.png?v=9" />
...
<body>
  ...
  <header class="app-header">
    ...
    <div class="header-left">
      <button id="menu-toggle" class="menu-toggle" type="button" aria-label="Apri menu">
        <div class="menu-icon">
          <span></span>
          <span></span>
          <span></span>
    ...
    <div class="header-center">
      <div class="header-brand">
        <img id="header-logo" class="header-logo" src="assets/favicon-192.png" alt="Logo azienda" />
        <div class="header-brand-text">
          <div id="header-location-name" class="header-location-name"></div>
          <div id="header-location-meta" class="header-location-meta"></div>
        </div>
      </div>
    </div>

    <div id="header-right" class="header-right">
      <div id="header-saluto" class="header-chip hide-mobile"></div>
      <div id="header-data" class="header-chip"></div>
      <div id="header-meteo" class="header-chip hide-mobile"></div>
    </div>
  ...
  <!-- CORE LOGIC -->
  <script src="js/stateActions.js?v=9"></script>
  <script src="js/ui-reparto.js?v=9"></script>
  <script src="js/db.js?v=9"></script>
  <script src="js/config/permessi.js?v=1"></script>
  <script src="js/utils/notify.js?v=1"></script>
  ...
  <script>
    function renderHeaderUI() {
      try {
        ...
        const nomeEl = document.getElementById("header-location-name");
        const metaEl = document.getElementById("header-location-meta");
        const salutoEl = document.getElementById("header-saluto");
        const dataEl = document.getElementById("header-data");
        const meteoEl = document.getElementById("header-meteo");
        ...
        if (metaEl) {
          ...
          if (
            sede?.nome &&
            azienda?.nome &&
            sede.nome !== azienda.nome
          ) {

            metaEl.textContent = azienda.nome;
        ...
        if (dataEl) {
          ...
          dataEl.textContent = oggi.toLocaleDateString(
            "it-IT",
            {
              weekday: "short",
              day: "2-digit",
    ...
    window.addEventListener("DOMContentLoaded", () => {
      ...
      setInterval(renderHeaderUI, 3000);
...
<!-- EVENT BUS -->
<script src="/js/services/events/eventBus.js?v=1"></script>

<!-- OPERATIONS -->
<script src="/js/services/operations/operationsEngine.js?v=1"></script>

<!-- PRINTING -->
<script src="/js/services/printing/printEngine.js?v=1"></script>
<script src="/js/services/printing/labelPrinter.js?v=1"></script>
<script src="/js/services/printing/kitchenPrinter.js?v=1"></script>

<!-- KDS -->
<script src="/js/services/kds/kdsEngine.js?v=1"></script>

  <!-- ROUTER (MODULE) -->
  <script
    type="module"
    src="js/router.js?v=12"
  ></script>

  <!-- TAWK RIMOSSO — rimane solo sul sito pubblico ristoflow-ai.com -->
  <script type="text/javascript">
    // Carica Tawk subito — nascosto su login, visibile dopo autenticazione
    var Tawk_API = Tawk_API || {};
    var Tawk_LoadStart = new Date();

    // Nasconde il widget sulle pagine pre-login
    Tawk_API.onLoad = function () {
      var hash = window.location.hash;
      var isPreLogin = !hash || hash === "#/" || hash.includes("login") || hash.includes("registrati");
      if (isPreLogin) {
        Tawk_API.hideWidget();
      }

      // Quando l'utente è loggato, mostra e identifica
      function aggiornaStatoTawk() {
        var user = window.state?.user;
        var azienda = window.state?.azienda;
        var h = window.location.hash;
        var preLogin = !h || h === "#/" || h.includes("login") || h.includes("registrati");

        if (user?.email && !preLogin) {
          Tawk_API.showWidget();
      ...
      // Controlla ogni volta che cambia la route
      window.addEventListener("hashchange", aggiornaStatoTawk);
      setInterval(aggiornaStatoTawk, 2000);
    };
    ...
    (function () {
      ...
      s1.async = true;
      s1.src = "https://embed.tawk.to/6a2a7b7c0514041c3544c5e9/1jqqv4occ";
      s1.charset = "UTF-8";
