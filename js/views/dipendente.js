...
// (FILE COMPLETO - VERSIONE CON ALLINEAMENTO AI INTEGRATO)
...
export async function render(container) {
  ...
  if (!user || !azienda) {
    container.innerHTML = `
      <div class="view">
        <h2 style="margin-top:0;">Errore</h2>
        <p class="small-muted">Sessione o azienda non disponibile.</p>
      </div>
    `;
    return;
  }
  ...
  if (!dipendenteId) {
    container.innerHTML = `<div class="view">Dipendente non trovato</div>`;
    return;
  }
  ...
  const [
    ...
    supabase.from("dipendenti").select("*").eq("id", dipendenteId).single(),
    supabase.from("sedi").select("id,nome"),
    supabase.from("reparti").select("id,nome"),
    supabase.from("dipendenti_valutazioni").select("*").eq("dipendente_id", dipendenteId).order("created_at", { ascending: false }),
    supabase.from("valutazioni_metriche").select("*").eq("attivo", true),
    calcolaPresenzaIbrida(azienda.id, dipendenteId),
    calcolaProduzioneIbrida(azienda.id, dipendenteId),
    calcolaQuizIbridi(azienda.id, dipendenteId)
  ]);
  ...
  const dip = dipRes.data;
  const valutazioni = valutazioniRes.data || [];
  const ultimaValutazione = valutazioni[0] || null;

  // 🔥 ALLINEAMENTO AI
  const alignment = calcolaAllineamento({
    azienda,
  ...
  const content = `
    ...
    ${createCard({
      title: "🧠 Allineamento aziendale",
      body: `
        <div style="display:grid; gap:10px;">
          ${scoreRow("Puntualità", formatAlignment(alignment.puntualita))}
          ${scoreRow("Collaborazione", formatAlignment(alignment.collaborazione))}
          ${scoreRow("Responsabilità", formatAlignment(alignment.responsabilita))}
          ${scoreRow("Qualità", formatAlignment(alignment.qualita))}
          ${scoreRow("Conoscenza", formatAlignment(alignment.conoscenza))}
        </div>
      `
    })}
...
function calcolaAllineamento({ azienda, presenza, produzione, quiz, valutazione }) {
  ...
  valori.forEach((v) => {
    ...
    const key = normalizeMetricName(v);

    if (key.includes("puntualita") || key.includes("presenza")) {
      result.puntualita = presenza.score >= 7 ? "alta" : presenza.score >= 5 ? "media" : "bassa";
    }

    if (key.includes("collaborazione")) {
      const val = valutazione?.punteggio_collaborazione;
      result.collaborazione = val >= 7 ? "alta" : val >= 5 ? "media" : "bassa";
    }

    if (key.includes("responsabilita")) {
      const val = valutazione?.punteggio_responsabilita;
      result.responsabilita = val >= 7 ? "alta" : val >= 5 ? "media" : "bassa";
    }

    if (key.includes("qualita")) {
      const val = valutazione?.punteggio_qualita || produzione.score;
      result.qualita = val >= 7 ? "alta" : val >= 5 ? "media" : "bassa";
    }

    if (key.includes("conoscenza")) {
      result.conoscenza = quiz.score >= 7 ? "alta" : quiz.score >= 5 ? "media" : "bassa";
    }
...
/* ================= MOCK FUNZIONI (LE TUE RESTANO UGUALI) ================= */

async function calcolaPresenzaIbrida() {
  return { score: 6 };
}

async function calcolaProduzioneIbrida() {
  return { score: 7 };
}

async function calcolaQuizIbridi() {
  return { score: 8 };
}
