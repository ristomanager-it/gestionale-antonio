export async function render(container) {
  ...
  let tags = []export async function render(container) {
              ...
              <div class="mk-card">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:10px;">
                  ...
                  <div style="display:flex;gap:6px;flex-shrink:0;flex-direction:column;">
                    ...
                    <button class="mk-btn btn-modifica" data-id="${c.id}" data-name="${c.name}" data-budget="${c.daily_budget||c.lifetime_budget||0}"
      ...
      content.querySelector('#btn-salva-modifica')?.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch(`${SUPABASE_URL}/functions/v1/meta-ads-proxy-ts`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${ANON_KEY}` },
            body: JSON.stringify({ endpoint: id, method: 'POST', params: { name: nome, daily_budget: budget } })
          });
          const data = await res.json();
