    ...
    <div style="max-width:900px;margin:0 auto;">
      ...
      <div style="display:flex;gap:0;overflow-x:auto;border-bottom:1px solid #e5e7eb;margin-bottom:24px;-webkit-overflow-scrolling:touch;scrollbar-width:none;">
        ${[
          ...
          { id:'catenarie',     icon:'🔗', label:'Catenarie'              },
        ].map(t => `
...
`;
  ...
  function switchTab(id) {
    ...
    switch(id) {
      ...
      case 'promo':        renderTabPromo(box);       break;
      case 'sondaggi':     renderTabSondaggi(box);    break;
      case 'catenarie':    renderTabCatenarie(box);   break;
    }
  }
  ...
  // TAB: PROMO
  // ════════════════════════════════════════
  async function renderTabPromo(box) {
    box.innerHTML = '<div style="color:#94a3b8;padding:20px;">Caricamento...</div>';

    const { data: promoList } = await supa()
      .from('promo')
      .select('*')
      .eq('azienda_id', aziendaId)
      .order('created_at', { ascending: false });

    const { data: tagList } = await supa()
      .from('tags')
      .select('id, nome')
      .eq('azienda_id', aziendaId)
      .order('nome');
    ...
    const TIPI = [
      { v:'sconto_perc', l:'Sconto %' },
      { v:'sconto_euro', l:'Sconto €' },
      { v:'omaggio',     l:'Omaggio' },
      { v:'2x1',         l:'2x1' },
    ];
    ...
    box.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:10px;">
        <div>
          <div style="font-size:17px;font-weight:700;color:#0f172a;">🎁 Promo & Offerte</div>
          <div style="font-size:13px;color:#64748b;">Crea promo personalizzate per segmento di clienti</div>
        </div>
        <button id="btn-nuova-promo" style="background:#0E5A7A;color:white;border:none;border-radius:10px;padding:9px 18px;cursor:pointer;font-size:13px;font-weight:600;">+ Nuova promo</button>
      </div>

      <!-- Lista promo -->
      <div id="lista-promo" style="margin-bottom:16px;"></div>

      <!-- Form promo -->
      <div id="form-promo" style="display:none;background:white;border:1px solid #e5e7eb;border-radius:14px;padding:20px;">
        <div style="font-size:16px;font-weight:700;margin-bottom:16px;" id="form-promo-title">Nuova promo</div>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,200px),1fr));gap:12px;margin-bottom:14px;">
          <div>
            <label style="font-size:12px;font-weight:600;color:#64748b;display:block;margin-bottom:4px;">Nome promo *</label>
            <input id="promo-nome" class="input" placeholder="Es. Benvenuto 10%" style="width:100%;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:12px;font-weight:600;color:#64748b;display:block;margin-bottom:4px;">Tipo</label>
            <select id="promo-tipo" class="input" style="width:100%;box-sizing:border-box;">
              ${TIPI.map(t => `<option value="${t.v}">${t.l}</option>`).join('')}
            </select>
          </div>
          <div>
            <label style="font-size:12px;font-weight:600;color:#64748b;display:block;margin-bottom:4px;">Valore</label>
            <input id="promo-valore" type="number" min="0" step="0.01" class="input" placeholder="Es. 10" style="width:100%;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:12px;font-weight:600;color:#64748b;display:block;margin-bottom:4px;">Validità (giorni)</label>
            <input id="promo-validita" type="number" min="1" value="30" class="input" style="width:100%;box-sizing:border-box;">
          </div>
        </div>
        <div style="margin-bottom:14px;">
          <label style="font-size:12px;font-weight:600;color:#64748b;display:block;margin-bottom:4px;">Descrizione</label>
          <textarea id="promo-desc" class="input" rows="2" placeholder="Descrizione breve della promo..." style="width:100%;box-sizing:border-box;resize:vertical;"></textarea>
        </div>
        <div style="margin-bottom:14px;">
          <label style="font-size:12px;font-weight:600;color:#64748b;display:block;margin-bottom:4px;">Messaggio WhatsApp</label>
          <textarea id="promo-msg-wa" class="input" rows="3" placeholder="Ciao {{nome}}! 🎁 Hai una promo per te: ..." style="width:100%;box-sizing:border-box;resize:vertical;font-family:monospace;font-size:12px;"></textarea>
          <div style="font-size:11px;color:#64748b;margin-top:4px;">Puoi usare: {{nome}}, {{promo_nome}}, {{promo_valore}}, {{scadenza}}</div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
          <div>
            <label style="font-size:12px;font-weight:600;color:#64748b;display:block;margin-bottom:4px;">🟢 Invia SOLO a clienti con questi tag</label>
            <div style="display:flex;flex-wrap:wrap;gap:6px;" id="promo-tag-inclusi">
              ${(tagList||[]).map(t => `<label style="display:flex;align-items:center;gap:4px;font-size:12px;cursor:pointer;background:#f1f5f9;padding:4px 8px;border-radius:6px;"><input type="checkbox" value="${t.id}" data-nome="${esc(t.nome)}" class="chk-tag-inclusi"> ${esc(t.nome)}</label>`).join('')}
            </div>
          </div>
          <div>
            <label style="font-size:12px;font-weight:600;color:#64748b;display:block;margin-bottom:4px;">🔴 NON inviare a clienti con questi tag</label>
            <div style="display:flex;flex-wrap:wrap;gap:6px;" id="promo-tag-esclusi">
              ${(tagList||[]).map(t => `<label style="display:flex;align-items:center;gap:4px;font-size:12px;cursor:pointer;background:#f1f5f9;padding:4px 8px;border-radius:6px;"><input type="checkbox" value="${t.id}" data-nome="${esc(t.nome)}" class="chk-tag-esclusi"> ${esc(t.nome)}</label>`).join('')}
            </div>
          </div>
        </div>
        <div id="promo-esito" style="font-size:13px;min-height:14px;margin-bottom:10px;"></div>
        <div style="display:flex;gap:8px;">
          <button id="btn-salva-promo" style="background:#0E5A7A;color:white;border:none;border-radius:10px;padding:10px 24px;cursor:pointer;font-size:14px;font-weight:600;">💾 Salva</button>
          <button id="btn-annulla-promo" style="background:#f1f5f9;color:#374151;border:none;border-radius:10px;padding:10px 18px;cursor:pointer;font-size:14px;">Annulla</button>
        </div>
      </div>
    `;

    function renderListaPromo() {
      const el = box.querySelector('#lista-promo');
      if (!promoList?.length) {
        el.innerHTML = '<div style="color:#94a3b8;font-size:13px;padding:8px 0;">Nessuna promo. Creane una per iniziare.</div>';
        return;
      }
      el.innerHTML = promoList.map(p => {
        const tipoLabel = TIPI.find(t=>t.v===p.tipo)?.l || p.tipo;
        const valore = p.tipo==='sconto_perc' ? `${p.valore}%` : p.tipo==='sconto_euro' ? `€${p.valore}` : tipoLabel;
        return `
          <div style="background:white;border:1px solid #e5e7eb;border-radius:12px;padding:14px 16px;margin-bottom:8px;display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
            ...
            <div style="flex:1;">
              ...
              <div style="font-size:12px;color:#64748b;margin-top:2px;">${tipoLabel} · <strong>${valore}</strong> · valida ${p.validita_giorni||30} giorni</div>
              ${p.descrizione ? `<div style="font-size:12px;color:#94a3b8;margin-top:2px;">${esc(p.descrizione)}</div>` : ''}
            </div>
            <div style="display:flex;gap:6px;align-items:center;">
              <span style="background:${p.attiva?'#dcfce7':'#fee2e2'};color:${p.attiva?'#15803d':'#dc2626'};padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;">${p.attiva?'Attiva':'Disattiva'}</span>
              <button data-del-promo="${p.id}" style="background:#fee2e2;border:none;padding:5px 10px;border-radius:8px;cursor:pointer;font-size:12px;color:#dc2626;">🗑</button>
            </div>
          </div>
        `;
      }).join('');
    ...
    box.querySelector('#btn-nuova-promo').addEventListener('click', () => {
      box.querySelector('#form-promo').style.display = '';
      box.querySelector('#form-promo-title').textContent = 'Nuova promo';
    });
    box.querySelector('#btn-annulla-promo').addEventListener('click', () => {
      box.querySelector('#form-promo').style.display = 'none';
    });

    box.querySelector('#btn-salva-promo').addEventListener('click', async () => {
      const esito = box.querySelector('#promo-esito');
      const nome = box.querySelector('#promo-nome').value.trim();
      if (!nome) { esito.textContent='❌ Nome obbligatorio'; esito.style.color='#dc2626'; return; }
      esito.textContent='Salvataggio...'; esito.style.color='#64748b';

      const tagInclusi = [...box.querySelectorAll('.chk-tag-inclusi:checked')].map(c=>c.dataset.nome);
      const tagEsclusi = [...box.querySelectorAll('.chk-tag-esclusi:checked')].map(c=>c.dataset.nome);

      const { data, error } = await supa().from('promo').insert({
        azienda_id: aziendaId,
        nome,
        descrizione: box.querySelector('#promo-desc').value.trim()||null,
        tipo: box.querySelector('#promo-tipo').value,
        valore: parseFloat(box.querySelector('#promo-valore').value)||null,
        validita_giorni: parseInt(box.querySelector('#promo-validita').value)||30,
        messaggio_wa: box.querySelector('#promo-msg-wa').value.trim()||null,
  ...
  // TAB: CATENARIE
  // ════════════════════════════════════════
  async function renderTabCatenarie(box) {
    box.innerHTML = '<div style="color:#94a3b8;padding:20px;">Caricamento...</div>';

    const [{ data: catenarie }, { data: promoList }, { data: sondaggiList }] = await Promise.all([
      supa().from('catenarie').select('*, catenarie_step(*)').eq('azienda_id', aziendaId).order('created_at', { ascending: false }),
      supa().from('promo').select('id, nome').eq('azienda_id', aziendaId).eq('attiva', true),
      supa().from('survey').select('id, titolo').eq('azienda_id', aziendaId),
    ]);

    const TRIGGER_TIPI = [
      { v:'prima_visita',  l:'🆕 Prima visita' },
      { v:'prenotazione',  l:'📅 Dopo prenotazione' },
      { v:'ordine',        l:'🛒 Dopo ordine/comanda' },
      { v:'compleanno',    l:'🎂 Compleanno cliente' },
    ];
    const STEP_TIPI = [
      { v:'attesa',      l:'⏱ Attesa' },
      { v:'sondaggio',   l:'📊 Invia sondaggio' },
      { v:'promo',       l:'🎁 Invia promo' },
      { v:'whatsapp',    l:'💬 Messaggio WhatsApp' },
      { v:'condizione',  l:'🔀 Condizione / Fork' },
    ];

    let catenariaAttiva = null;
    let steps = [];

    box.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:10px;">
        <div>
          <div style="font-size:17px;font-weight:700;color:#0f172a;">🔗 Catenarie automatiche</div>
          <div style="font-size:13px;color:#64748b;">Sequenze automatiche di fidelizzazione clienti</div>
        </div>
        <button id="btn-nuova-catenaria" style="background:#0E5A7A;color:white;border:none;border-radius:10px;padding:9px 18px;cursor:pointer;font-size:13px;font-weight:600;">+ Nuova catenaria</button>
      </div>
      ...
      <div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,2fr);gap:16px;">
        ...
        <!-- Editor -->
        <div id="editor-catenaria" style="display:none;">
          <div style="background:white;border:1px solid #e5e7eb;border-radius:14px;padding:16px;margin-bottom:12px;">
            <div style="font-size:15px;font-weight:700;margin-bottom:12px;" id="cat-editor-title">Nuova catenaria</div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
              <div>
                <label style="font-size:11px;font-weight:700;color:#64748b;display:block;margin-bottom:4px;">Nome *</label>
                <input id="cat-nome" class="input" placeholder="Es. Benvenuto prima visita" style="width:100%;box-sizing:border-box;">
              </div>
              <div>
                <label style="font-size:11px;font-weight:700;color:#64748b;display:block;margin-bottom:4px;">Trigger — quando parte</label>
                <select id="cat-trigger" class="input" style="width:100%;box-sizing:border-box;">
                  ${TRIGGER_TIPI.map(t=>`<option value="${t.v}">${t.l}</option>`).join('')}
                </select>
              </div>
            </div>
            <textarea id="cat-desc" class="input" rows="2" placeholder="Descrizione (opzionale)" style="width:100%;box-sizing:border-box;resize:vertical;"></textarea>
          </div>

          <!-- Step builder -->
          <div style="background:white;border:1px solid #e5e7eb;border-radius:14px;padding:16px;margin-bottom:12px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
              <div style="font-size:14px;font-weight:700;">📋 Step sequenza</div>
              <button id="btn-aggiungi-step" style="background:#f0f9ff;color:#0E5A7A;border:1px solid #bae6fd;border-radius:8px;padding:6px 14px;cursor:pointer;font-size:12px;font-weight:600;">+ Aggiungi step</button>
            </div>
            <div id="lista-steps"></div>

            <!-- Form step -->
            <div id="form-step" style="display:none;background:#f8fafc;border-radius:10px;padding:14px;margin-top:10px;">
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
                <div>
                  <label style="font-size:11px;font-weight:700;color:#64748b;display:block;margin-bottom:4px;">Tipo step</label>
                  <select id="step-tipo" class="input" style="width:100%;box-sizing:border-box;">
                    ${STEP_TIPI.map(t=>`<option value="${t.v}">${t.l}</option>`).join('')}
                  </select>
                </div>
                <div>
                  <label style="font-size:11px;font-weight:700;color:#64748b;display:block;margin-bottom:4px;">Delay (giorni dopo step precedente)</label>
                  <input id="step-delay" type="number" min="0" value="1" class="input" style="width:100%;box-sizing:border-box;">
                </div>
              </div>
              <div>
                <label style="font-size:11px;font-weight:700;color:#64748b;display:block;margin-bottom:4px;">Nome step</label>
                <input id="step-nome" class="input" placeholder="Es. Invia sondaggio feedback" style="width:100%;box-sizing:border-box;margin-bottom:10px;">
              </div>
              <div id="step-sondaggio-wrap" style="display:none;margin-bottom:10px;">
                <label style="font-size:11px;font-weight:700;color:#64748b;display:block;margin-bottom:4px;">Sondaggio da inviare</label>
                <select id="step-survey-id" class="input" style="width:100%;box-sizing:border-box;">
                  <option value="">— Seleziona sondaggio —</option>
                  ${(sondaggiList||[]).map(s=>`<option value="${s.id}">${esc(s.titolo)}</option>`).join('')}
                </select>
              </div>
              <div id="step-promo-wrap" style="display:none;margin-bottom:10px;">
                <label style="font-size:11px;font-weight:700;color:#64748b;display:block;margin-bottom:4px;">Promo da inviare</label>
                <select id="step-promo-id" class="input" style="width:100%;box-sizing:border-box;">
                  <option value="">— Seleziona promo —</option>
                  ${(promoList||[]).map(p=>`<option value="${p.id}">${esc(p.nome)}</option>`).join('')}
                </select>
              </div>
              <div id="step-msg-wrap" style="display:none;margin-bottom:10px;">
                <label style="font-size:11px;font-weight:700;color:#64748b;display:block;margin-bottom:4px;">Messaggio WhatsApp</label>
                <textarea id="step-messaggio" class="input" rows="3" placeholder="Ciao {{nome}}! ..." style="width:100%;box-sizing:border-box;resize:vertical;font-size:12px;font-family:monospace;"></textarea>
              </div>
              <div id="step-condizione-wrap" style="display:none;margin-bottom:10px;">
                <label style="font-size:11px;font-weight:700;color:#64748b;display:block;margin-bottom:4px;">Condizione (es. se NPS ≤ 3 vai a step X)</label>
                <div style="display:grid;grid-template-columns:1fr 80px 80px;gap:8px;">
                  <input id="step-cond-campo" class="input" placeholder="Campo es. nps_score" style="box-sizing:border-box;">
                  <select id="step-cond-op" class="input" style="box-sizing:border-box;">
                    <option value="<=">≤</option><option value=">=">≥</option><option value="==">==</option><option value="!=">!=</option>
                  </select>
                  <input id="step-cond-val" class="input" placeholder="3" style="box-sizing:border-box;">
                </div>
              </div>
              <div style="display:flex;gap:8px;">
                <button id="btn-salva-step" style="background:#0E5A7A;color:white;border:none;border-radius:8px;padding:8px 18px;cursor:pointer;font-size:13px;font-weight:600;">Aggiungi step</button>
                <button id="btn-annulla-step" style="background:#f1f5f9;color:#374151;border:none;border-radius:8px;padding:8px 14px;cursor:pointer;font-size:13px;">Annulla</button>
              </div>
            </div>
          </div>

          <div id="cat-esito" style="font-size:13px;min-height:14px;margin-bottom:10px;"></div>
          <div style="display:flex;gap:8px;">
            <button id="btn-salva-catenaria" style="background:#0E5A7A;color:white;border:none;border-radius:10px;padding:10px 24px;cursor:pointer;font-size:14px;font-weight:600;">💾 Salva catenaria</button>
            <button id="btn-annulla-catenaria" style="background:#f1f5f9;color:#374151;border:none;border-radius:10px;padding:10px 18px;cursor:pointer;font-size:14px;">Annulla</button>
          </div>
        </div>
      </div>
    `;

    function renderListaCatenarie() {
      const el = box.querySelector('#lista-catenarie');
      if (!catenarie?.length) {
        el.innerHTML = '<div style="color:#94a3b8;font-size:13px;padding:8px 0;">Nessuna catenaria. Creane una.</div>';
        return;
      }
      el.innerHTML = (catenarie||[]).map(c => {
        const trig = TRIGGER_TIPI.find(t=>t.v===c.trigger_tipo)?.l || c.trigger_tipo;
        const nStep = c.catenarie_step?.length || 0;
        return `
          <div data-cat="${c.id}" style="background:white;border:1px solid ${catenariaAttiva?.id===c.id?'#0E5A7A':'#e5e7eb'};border-radius:12px;padding:12px;margin-bottom:8px;cursor:pointer;">
            <div style="font-weight:700;font-size:13px;">${esc(c.nome)}</div>
            <div style="font-size:11px;color:#64748b;margin-top:2px;">${trig} · ${nStep} step</div>
            <span style="font-size:11px;background:${c.attiva?'#dcfce7':'#f1f5f9'};color:${c.attiva?'#15803d':'#374151'};padding:2px 8px;border-radius:20px;">${c.attiva?'Attiva':'Disattiva'}</span>
          </div>
        `;
      }).join('');
      el.querySelectorAll('[data-cat]').forEach(card => {
        card.addEventListener('click', () => apriEditorCat(catenarie.find(c=>c.id===card.dataset.cat)));
      });
    }

    function apriEditorCat(catenaria = null) {
      catenariaAttiva = catenaria;
      steps = catenaria?.catenarie_step ? [...catenaria.catenarie_step].sort((a,b)=>a.ordine-b.ordine) : [];
      box.querySelector('#editor-catenaria').style.display='';
      box.querySelector('#cat-editor-title').textContent = catenaria ? 'Modifica catenaria' : 'Nuova catenaria';
      box.querySelector('#cat-nome').value = catenaria?.nome || '';
      box.querySelector('#cat-trigger').value = catenaria?.trigger_tipo || 'prima_visita';
      box.querySelector('#cat-desc').value = catenaria?.descrizione || '';
      renderSteps();
    }

    function renderSteps() {
      const el = box.querySelector('#lista-steps');
      if (!steps.length) {
        el.innerHTML = '<div style="color:#94a3b8;font-size:12px;padding:8px 0;">Nessuno step. Aggiungine uno.</div>';
        return;
      }
      el.innerHTML = steps.map((s, idx) => {
        const tipoLabel = STEP_TIPI.find(t=>t.v===s.tipo)?.l||s.tipo;
        return `
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
            <div style="width:24px;height:24px;background:#0E5A7A;color:white;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;flex-shrink:0;">${idx+1}</div>
            <div style="flex:1;background:white;border:1px solid #e5e7eb;border-radius:8px;padding:8px 12px;">
              <div style="font-size:13px;font-weight:600;">${esc(s.nome||tipoLabel)}</div>
              <div style="font-size:11px;color:#64748b;">${tipoLabel} · delay ${s.delay_giorni||0}gg</div>
            </div>
            ${idx>0?`<div style="width:2px;height:16px;background:#e5e7eb;margin:-14px 0 -14px 11px;position:absolute;"></div>`:''}
            <button data-del-step="${idx}" style="background:#fee2e2;border:none;padding:4px 8px;border-radius:6px;cursor:pointer;font-size:11px;color:#dc2626;">✕</button>
          </div>
        `;
      }).join('');
      el.querySelectorAll('[data-del-step]').forEach(btn => {
        btn.addEventListener('click', () => { steps.splice(parseInt(btn.dataset.delStep),1); renderSteps(); });
      });
    }

    // Tipo step → mostra/nascondi campi
    box.querySelector('#step-tipo').addEventListener('change', (e) => {
      const tipo = e.target.value;
      box.querySelector('#step-sondaggio-wrap').style.display = tipo==='sondaggio' ? '' : 'none';
      box.querySelector('#step-promo-wrap').style.display = tipo==='promo' ? '' : 'none';
      box.querySelector('#step-msg-wrap').style.display = tipo==='whatsapp' ? '' : 'none';
      box.querySelector('#step-condizione-wrap').style.display = tipo==='condizione' ? '' : 'none';
    });

    box.querySelector('#btn-aggiungi-step').addEventListener('click', () => {
      box.querySelector('#form-step').style.display='';
    });
    box.querySelector('#btn-annulla-step').addEventListener('click', () => {
      box.querySelector('#form-step').style.display='none';
    });
    box.querySelector('#btn-salva-step').addEventListener('click', () => {
      const nome = box.querySelector('#step-nome').value.trim() || box.querySelector('#step-tipo').value;
      const tipo = box.querySelector('#step-tipo').value;
      steps.push({
        nome,
        tipo,
        ordine: steps.length+1,
        delay_giorni: parseInt(box.querySelector('#step-delay').value)||0,
        survey_id: box.querySelector('#step-survey-id').value||null,
        promo_id: box.querySelector('#step-promo-id').value||null,
        messaggio: box.querySelector('#step-messaggio').value.trim()||null,
        condizione_campo: box.querySelector('#step-cond-campo').value.trim()||null,
        condizione_operatore: box.querySelector('#step-cond-op').value||null,
        condizione_valore: box.querySelector('#step-cond-val').value.trim()||null,
      });
      renderSteps();
      box.querySelector('#form-step').style.display='none';
      box.querySelector('#step-nome').value='';
    });

    box.querySelector('#btn-nuova-catenaria').addEventListener('click', () => apriEditorCat(null));
    box.querySelector('#btn-annulla-catenaria').addEventListener('click', () => {
      box.querySelector('#editor-catenaria').style.display='none';
    });

    box.querySelector('#btn-salva-catenaria').addEventListener('click', async () => {
      const esito = box.querySelector('#cat-esito');
      const nome = box.querySelector('#cat-nome').value.trim();
      if (!nome) { esito.textContent='❌ Nome obbligatorio'; esito.style.color='#dc2626'; return; }
      esito.textContent='Salvataggio...'; esito.style.color='#64748b';
      ...
      const payload = {
        azienda_id: aziendaId,
        nome,
        descrizione: box.querySelector('#cat-desc').value.trim()||null,
        trigger_tipo: box.querySelector('#cat-trigger').value,
        attiva: true,
      };

      let catId = catenariaAttiva?.id;
      if (catId) {
        await supa().from('catenarie').update(payload).eq('id', catId);
      } else {
        const { data, error } = await supa().from('catenarie').insert(payload).select('*').single();
        if (error) { esito.textContent='❌ '+error.message; esito.style.color='#dc2626'; return; }
        catId = data.id;
        catenarie.unshift({ ...data, catenarie_step: [] });
      }

      // Salva step
      if (steps.length) {
        await supa().from('catenarie_step').delete().eq('catenaria_id', catId);
        await supa().from('catenarie_step').insert(steps.map((s,i)=>({
          ...s, catenaria_id: catId, azienda_id: aziendaId, ordine: i+1
        })));
      }

      esito.textContent='✅ Catenaria salvata!'; esito.style.color='#15803d';
      renderListaCatenarie();
